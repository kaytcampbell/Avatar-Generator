...
export async function POST(request: Request) {
  ...
  const rateLimit = checkRateLimit(`upload:${data.user.id}`, UPLOAD_LIMIT, UPLOAD_WINDOW_MS);
  ...
  try {
    ...
    const uploaded = await uploadImageBuffer(buffer, { folder: 'avatar-gen/stylized', mimeType: photo.type });
    return NextResponse.json({ id: `upload-${Date.now()}`, ...uploaded });
  } catch (err) {
